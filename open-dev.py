...
def get_all_objects(frame, known_parking, types=[ObjectType.CAR.value], cof=0.25,
                    iou=0.45) -> CameraData:
    ...
    xyxy_ce = known_parking.get('xyxy') if len(known_parking) > 0 else []
    max_height = 0
    ...
    # cv2.putText(frame, f'{int(height * scaling)}', (cent_xyxy[0] - 50, int(cent_xyxy[1] - count )),
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0),
    #             2)
    # cv2.putText(frame, f'{int(yc)}', (cent_xyxy[0], int(cent_xyxy[1] - count)),
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
    # print(f"max height {max_height}")

    found_parking = set()
    not_found_parking = set()
    for result in results[0].boxes:
        if result.cls in types:  # Class 0 corresponds to "person" in COCO dataset
            ...
            for parking in known_parking.get('list'):
                ...
                if distance <= 15:
                    founds = True
                    found_parking.add((xc, yc))
                    height = y2 - y1
                    scaling = height / max_height
                    height_scaled = abs((height - (height * scaling)) / 2)
                    print(f"height_scaled{height_scaled}")
                    y11 = int(y1 + height_scaled)
                    y22 = int(y2 - height_scaled)

                    cv2.rectangle(frame, (x1, y11), (x2, y22), (0, 0, 255), 1)  # Green box, thickness 2
                    cv2.putText(frame, f'{height}', (x1 - 40, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 0), 1)
                    print(f"height {height}")
                    print(f"cords {(x1, y1), (x2, y2)}")
                    print(f"scaled {(y11, y22)}")

                    print(f"Breaking inner loop and skipping outer iteration. {(xc, yc)}")
                    break
            #
            if not founds:
                not_found_parking.add((xc, yc))
                color = (0, 0, 255)
                radius = 5
                thickness = 1
    ...
    print(not_found_parking)
    # for xc, yc in not_found_parking:
    #     cv2.circle(frame, (int(xc), int(yc)), radius, (255, 0, 0 ), 1)

    for xc, yc in known_parking.get('list'):
        found = False
        for found_park in found_parking:
            distance = euclidean((xc, yc), found_park)
            if distance <= 15:
                found = True
        ...
        if not found:
            ...
            height_scaled = abs((height - (height * scaling)) / 2)
            print(f"height_scaled{height_scaled}")
            y11 = int(cent_xyxy[1] + height_scaled)
            y22 = int(cent_xyxy[3] - height_scaled)
            y11 = int(cent_xyxy[1])
            y22 = int(cent_xyxy[3])

            # cv2.rectangle(frame, (cent_xyxy[0], y11), (cent_xyxy[2], y22), (0, 255, 0), 1)  # Green box, thickness 2

    for xc, yc in found_parking:
        cv2.circle(frame, (int(xc), int(yc)), radius, (255, 255, 255), 1)
...
def get_camera_data(camera_name, camera_id, image_url, known_parking) -> CameraData:
    ...
    cv2.resizeWindow("Image", 1800, 1600)
...
def run_count():
    ...
    while True:
        ...
        for camera in cameras:
            ...
            known_parking = {"xyxy": [], "list": []}
            if all_cameras and camera_id in all_cameras:
                known_parking = all_cameras[camera_id].cars.knownParking
                if known_parking and "xyxy" in known_parking:
